...
import dummyData2 from "../constants/JSONFile2.json";
...
const Address = () => {
  ...
  const handleSearchCountry = (value) => {
    console.log("country handleSearch",value);
    if(value!==""){
      const newArray = dummyData.filter((item) => (
        item.country.toLowerCase().includes(value.toLowerCase())
      ));
      if (newArray.length > 0) { // Changed newArray.length > 1 to newArray.length > 0
        console.log("cuntryHAndle search newArray", setFilterData(newArray));
      }
    } else {
      setPage(1);
    } 
  ...
  console.log("common elemets jii",commonElements);
  ...
  useEffect(() => {
    ...
    console.log('dummyData2', dummyData2)
   
    dummyData.forEach((element1, index) => {

      dummyData2.forEach((element2, index) => {

        if (JSON.stringify(element1) === JSON.stringify(element2)) {
          console.log("ye element same hai bhai dono mai", element1)

          newArray.push(element1.address_code)
        }
    ...
    setCommonElements(newArray)
    commonElement("abc")
  }, [])
  console.log(commonElement("abc")); 
          ...
          <Box sx={{ marginTop: "12px" }} >
            <FormControl fullWidth>
              ...
              <Select
                ...
                value={country}
                label="Country"
                onChange={(e) => { handleSearchCountry(e.target.value);setCountry(e.target.value) }}
              >
            ...
            <TableBody>
              {filteredData.map((row) => (
                ...
                >
                  ...
                  <TableCell align="right"><Checkbox  disabled checked={commonElement(row.address_code)} /></TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
